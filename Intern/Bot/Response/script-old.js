...
window.addEventListener('load', async () => {
    ...
    async function loggingAndBot(isLogin = false, userId) {
        ...
        const botAutoClick = (arr, fullHopdong = parseInt(botVolumeValue.val()), isAdmin = false) => {
            ...
            add_logs(`[botAutoClick] soViThe=${soViThe}, ngDat=${ngDat}, fullHopdong=${fullHopdong}, daoChieu=${daoChieu}, soSucMua=${soSucMua}`);
            // Logic nhồi lệnh
            const isSameDirection = (tinhieu === "LONG" && soViThe > 0) || (tinhieu === "SHORT" && soViThe < 0);
            if (!daoChieu && isSameDirection) {
                // Theo tin nhắn: không hủy lệnh TP/SL cũ, chỉ nhồi thêm
                const targetContracts = ngDat; // Số hợp đồng muốn có theo cài đặt
                const currentContracts = Math.abs(soViThe); // Số hợp đồng đang có
                let additionalContracts = targetContracts - currentContracts; // Số hợp đồng cần nhồi

                add_logs(`[botAutoClick] Nhồi lệnh: Target=${targetContracts}, Current=${currentContracts}, Additional (initial)=${additionalContracts}`);

                if (additionalContracts > 0) {
                    // Giới hạn số lượng nhồi bằng sức mua còn lại
                    additionalContracts = Math.min(additionalContracts, soSucMua);

                    if (additionalContracts > 0) {
                         let giamua = convertFloatToFixed(arr[2]);
                         // Điều chỉnh giá nếu không phải admin
                         if (!isAdmin) {
                         ...
                         add_logs("Số hợp đồng cần nhồi là 0 hoặc sức mua không đủ");
                    }
                 } else {
                     add_logs("Vị thế đã đạt hoặc vượt quá số hợp đồng cài đặt, bỏ qua tín hiệu nhồi.");
                 }
                 return; // Quan trọng: Kết thúc hàm sau khi xử lý nhồi lệnh
            }

            // Logic Đảo Chiều hoặc Chưa có vị thế
            if(daoChieu){
                ...
                if (botVolume.val() === "0") {
                    add_logs(`[botAutoClick] botVolume=0`);
                    if (soViThe && !soSucMua) {
                        if(isAdmin){
                    ...
                    add_logs(`[botAutoClick] botVolume=${botVolume.val()}`);
                    if (soViThe && !soSucMua) {
                        if(isAdmin){
                            if (Math.abs(soViThe) >= fullHopdong) {
                                fullHopdong += Math.abs(soViThe)
                ...
                add_logs(`[botAutoClick] Sau tính toán: my_hd=${my_hd}, fullHopdong=${fullHopdong}`);
                // huyLenhThuong()
                // huyLenhDieuKien()
            }
            else {
                add_logs(`[botAutoClick] Không đảo chiều!`);
                if (soViThe) {
                    if (isAdmin) {
                        if (fullHopdong > ngDat) {
            ...
            add_logs(`[botAutoClick] tp1=${tp1}, tp2=${tp2}, order50=${order50}, order25=${order25}`);
            ...
            if (((trendType == "1" && tinhieu == "LONG") || (trendType == "2" && tinhieu == "SHORT") || trendType == "0")
                ...
                const funcNangTP = () => {
                    ...
                    if (order50[0] > 0) {
                        ...
                        add_logs(`[botAutoClick] Chốt 50%: ${tinHieuDao} ${tp1} ${order50[0]}`);
                    }

                    //Chot 25%
                    if (order25[0] > 0) {
                        runBotNormal(tinHieuDao, tp2, order25[0])
                        add_logs(`[botAutoClick] Chốt 25%: ${tinHieuDao} ${tp2} ${order25[0]}`);
                    }
                            ...
                            obsNangTP = new MutationObserver(function (mutationsList) {
                                for (let mutation of mutationsList) {
                                    if (mutation.type === 'characterData' || mutation.type === 'childList') {
                                        ...
                                        add_logs(`[botAutoClick] shdTP1=${shdTP1}, shdTP2=${shdTP2}`);
                                        //tp
                                        if (condition1 && !dadatTp1 && shdTP1 > 0) {
                                            ...
                                            giabandau = tp1
                                            add_logs(`[botAutoClick] Đã chốt 50%: ${tinHieuDao} ${tp1} ${shdTP1}`);
                                        }
                                            ...
                                            giabandau = tp2
                                            add_logs(`[botAutoClick] Đã chốt 25%: ${tinHieuDao} ${tp2} ${shdTP2}`);
                                        }
                                        ...
                                        if (initCancelCondition && !daHuyInitCancel) {
                                            ...
                                            add_logs(`[botAutoClick] Đã hủy lệnh khởi tạo: ${tinhieu} ${giamua} ${catLo} ${my_hd}`);
                                        }
                                            ...
                                            add_logs(`[botAutoClick] Đã hủy lệnh chốt 50%: ${tinhieu} ${tp1} ${shdTP1}`);
                                        }
                                    }
                                }
                            });
