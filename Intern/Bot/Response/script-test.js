...
window.addEventListener('load', async () => {
    ...
    async function loggingAndBot(isLogin = false, userId) {
        ...
        const huyViTheHienTai = () => {
            huyLenhThuong()
            add_logs("Huỷ lệnh trong huyViTheHienTai")
            huyLenhDieuKien()
            ...
            if(daoChieu){
                ...
                if (botVolume.val() === "0") {
                    ...
                    if (soViThe && !soSucMua) {
                        ...
                        const setupHopDong = ngDat                   
                        const viTheHienTai = Math.abs(soViThe)        
                        const hopDongDaTP = setupHopDong - viTheHienTai  
                        const percentTP = hopDongDaTP / setupHopDong      
                        let tiLeDao = 2
                    
                        if (percentTP >= 0.75) {
                            tiLeDao = 1.2
                        } else if (percentTP >= 0.5) {
                            tiLeDao = 1.5
                        }
                    
                        const hopDongDao = Math.round(setupHopDong * tiLeDao)

                        my_hd = hopDongDao - viTheHienTai
                        fullHopdong = hopDongDao
                    
                        add_logs(`Đã TP ${Math.round(percentTP * 100)}% —> Đảo với tỷ lệ ${tiLeDao}x = ${hopDongDao} hợp đồng`)
                    }
                }
                
                huyLenhThuong()
                add_logs("Huy lệnh trong đảo chiều")
                huyLenhDieuKien()
            }
            else {
                add_logs("Không đảo chiều!")
                if (soViThe) {
                    if (isAdmin) {
                        if (fullHopdong > ngDat) {
                        ...
                        if (trangthaiBanDau == 'Đã khớp') {
                            ...
                            const huyLenhSau90s = setTimeout(() => {
                                ...
                                if (trangthai == 'Chờ khớp') {
                                    ...
                                    huyLenhThuong()
                                    add_logs("Huỷ lệnh do hết thời gian chờ")
                                }
                            }, 90000)
