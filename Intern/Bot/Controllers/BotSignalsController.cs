...
namespace Bot.Controllers
{
    ...
    public class BotSignalsController : ControllerBase
    {
        ...
        // 🧠 Lưu lại tín hiệu cuối cùng trong ngày để tránh gửi trùng
        private static string? _lastSignal = null;
        private static DateTime _lastSignalTime = DateTime.MinValue;
        ...
        public async Task<IActionResult> SendMessage([FromForm] SendMessageRequest request)
        {
            try
            {
                ...
                // 🔄 Nếu đã qua ngày mới → reset tín hiệu cũ
                if (_lastSignalTime.Date != now.Date)
                {
                    Console.WriteLine("📅 Sang ngay moi → reset trang thai tin hieu cu");
                    _lastSignal = null;
                }

                // ⛔ Nếu tín hiệu giống tín hiệu cuối cùng trong cùng ngày → bỏ qua
                if (_lastSignal == signal)
                {
                    Console.WriteLine($"⏸ Bo qua tin hieu trung trong ngay: {_lastSignal} (thoi gian: {_lastSignalTime})");
                    return Ok(new { status = "ignored", reason = "duplicate_signal_same_day" });
                }
                ...
                Console.WriteLine($"✅ Gui tin hieu moi: {signal}");
                return Ok(new { status = "sent", signal });
            }
            catch (Exception ex)
            {
